...
# 表示日数選択
""")
option = st.sidebar.radio("表示期間を選択してください", ("day", "month", "year"), key='period_radio_unique')

if option == "day":
    date = st.sidebar.selectbox("日数を選択してください", ("1", "5"), key='day_selectbox_unique')
    period = f"{date}d"
    st.write(f"""### 過去 **{date}日間**の金価格""")
elif option == "month":
    date = st.sidebar.selectbox("月数を選択してください", ("1", "3", "6"), key='month_selectbox_unique')
    period = f"{date}mo"
    st.write(f"""### 過去 **{date}月間**の金価格""")
elif option == "year":
    date = st.sidebar.selectbox("年数を選択してください", ("1", "2", "5", "10"), key='year_selectbox_unique')
    period = f"{date}y"
...
if option == "day":
        ...
        #（最小値，最大値，デフォルト）
        ymin,ymax = st.sidebar.slider("範囲を指定してください", 0,20000,(11000,12000))
if option == "month":
    st.sidebar.write("""## 金価格の範囲指定""")
    #（最小値，最大値，デフォルト）
...
if option == "year":
    st.sidebar.write("""## 金価格の範囲指定""")
    #（最小値，最大値，デフォルト）
    ymin,ymax = st.sidebar.slider("範囲を指定してください", 0,20000,(3000,15000))
